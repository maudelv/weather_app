          ...
          <div class="current-weather-details md:items-center">
            <div class="temperature-main text-center md:text-left mb-6 md:mb-0 flex-shrink-0">
              ...
              <%= if @weather.current_weather.icon && @weather.current_weather.icon != "" do %>
                <img src={"https://openweathermap.org/img/wn/#{@weather.current_weather.icon}@2x.png"} alt="weather icon" class="w-16 h-16 mx-auto md:mx-0 md:inline-block md:ml-4 align-middle">
              <% end %>
            </div>
            ...
            <div class="weather-grid grid grid-cols-1 sm:grid-cols-4 gap-4 flex-grow">
              <%= for day <- Enum.take(@weather.weekly_weather, 1) do %>
                <div class="weather-item bg-blue-50 p-4 rounded-md shadow-sm">
                  <span class="weather-label block text-sm font-medium text-blue-600 mb-1">Temperatura máxima:</span>
                  <span class="weather-value text-m font-semibold text-blue-800"><%= round(day.temp_max || day.temperature) %>°</span>
                </div>

                <div class="weather-item bg-blue-50 p-4 rounded-md shadow-sm">
                  <span class="weather-label block text-sm font-medium text-blue-600 mb-1">Temperatura mínima</span>
                  <span class="weather-value text-m font-semibold text-blue-800"><%= round(day.temp_min || day.temperature) %>°</span>
                </div>
              <% end %>

              <div class="weather-item bg-blue-50 p-4 rounded-md shadow-sm">
                <span class="weather-label block text-sm font-medium text-blue-600 mb-1">Humedad:</span>
                <span class="weather-value text-m font-semibold text-blue-800"><%= @weather.current_weather.humidity %>%</span>
              </div>

              <div class="weather-item bg-blue-50 p-4 rounded-md shadow-sm">
                <span class="weather-label block text-sm font-medium text-blue-600 mb-1">Descripción:</span>
                <span class="weather-value text-m font-semibold text-blue-800 capitalize"><%= @weather.current_weather.description %></span>
              </div>
            ...
            <%= for hour <- Enum.take(@weather.hourly_weather, 24) do %>
              <div class="hourly-item flex-shrink-0 w-28 bg-blue-50 p-3 rounded-md border border-blue-200 text-center shadow-sm">
                <div class="hour-time text-sm font-semibold text-blue-600">
                  <%= DateTime.from_unix!(hour.time) |> DateTime.to_time() |> Time.to_string() |> String.slice(0..4) %>
                </div>
                <div class="hour-icon my-2">
                  <%= if hour.icon && hour.icon != "" do %>
                    <img src={"https://openweathermap.org/img/wn/#{hour.icon}@2x.png"} alt="weather icon" class="w-12 h-12 mx-auto">
                  <% end %>
                </div>
              ...
              <div class="daily-item flex-shrink-0 w-40 bg-blue-50 p-4 rounded-md border border-blue-200 text-center shadow-sm">
                <div class="day-info mb-1">
                  <div class="day-name text-md font-bold text-blue-700">
                    <%= DateTime.from_unix!(day.date) |> DateTime.to_date() |> Date.day_of_week() |> day_name() %>
                  </div>
                ...
                <div class="day-icon my-2">
                  <%= if day.icon do %>
                    <img src={"https://openweathermap.org/img/wn/#{day.icon}@2x.png"} alt="weather icon" class="w-16 h-16 mx-auto">
                  <% end %>
                </div>
                ...
                <div class="day-temps text-2xl font-bold text-blue-800 my-2">
                  ...
                  <span class="temp-min text-blue-600"><%= round(day.temp_min || day.temperature) %>°</span>
                </div>
